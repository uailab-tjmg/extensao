...
export function observeDOMChanges(): MutationObserver | undefined {
  ...
  const observer = new MutationObserver((mutationsList) => {
    const shouldProcess = mutationsList.some(
      (mutation) =>
        mutation.type === "childList" &&
        Array.from(mutation.addedNodes).some(
          (node) =>
            node.nodeType === Node.ELEMENT_NODE &&
            ((node as Element).matches(".anexos") ||
              (node as Element).querySelector(".anexos")),
        ),
    )
    ...
    if (shouldProcess) {
      document.querySelectorAll("#divTimeLine .anexos").forEach((anexos) => {
        if (
          !anexos.previousElementSibling?.querySelector<HTMLInputElement>(
